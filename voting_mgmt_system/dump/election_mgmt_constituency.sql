...
CREATE TABLE `constituency` (
  ...
  `C_District` varchar(45) DEFAULT NULL,
  PRIMARY KEY (`C_Code`),
  KEY `Constituency_fk_idx` (`C_District`),
  CONSTRAINT `c fk ` FOREIGN KEY (`C_Code`) REFERENCES `province` (`district`) ON DELETE RESTRICT ON UPDATE RESTRICT,
  CONSTRAINT `cont fk ` FOREIGN KEY (`C_District`) REFERENCES `province` (`district`) ON DELETE RESTRICT ON UPDATE CASCADE
